...
| **Log-Sum-Exp Trick** | A numerical stability technique that subtracts a constant (usually the maximum logit) before exponentiation to prevent overflow without changing results. *(D2L 3.7)* |
...
| **Momentum** | An optimization technique that accumulates a running average of past gradients to smooth updates and reduce oscillations in narrow valleys. *(D2L 12.4)* |
...
| **Parameter Sharing** | Reusing the same parameters across different spatial locations in a convolutional layer to reduce model size and enforce structure. *(D2L 19.1–19.2)* |
